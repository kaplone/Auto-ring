...
def dvdaut(repertoire_de_travail, nom_export, lettre_1, lettre_2) :
    ...
    fichier_inter_debut = """<dvdauthor dest="%s/export_dvd">
    <vmgm>
...
-->
    ...
    for x in lecture_tc_chapitres :
        if x[0] in ["/", "[", "F"]:
            ...
            if x.split(":")[0] != "00" :
                y = ",%s:%s:%s" % (x.split(":")[0],x.split(":")[1],x.split(":")[2])
            else :
                y = ",%s:%s" % (x.split(":")[1],x.split(":")[2])
            tc = tc + y
...
######### page des bonus ########
    ...
    final_titleset = """            </pgc>
        ...
        <titles>
            ...
            <audio format="ac3" lang="FR"></audio>
            <pgc>
                <vob file="%s/generique_%s.mpg"></vob>
                <post> jump title 2; </post>
            </pgc>
            <pgc>
                <vob file="%s/%s_womble_mplex.mpg" chapters="%s"></vob>
                <post>call menu entry root;</post>
            </pgc>
            <pgc>
                <vob file="%s/%s_bonus_%s.mpg"></vob>
                <post>call menu entry root;</post>
            </pgc> 
            <pgc>
                <vob file="%s/%s_bonus_%s.mpg"></vob>
