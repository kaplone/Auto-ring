...
fond = pdb.file_jpeg_load("/home/autor/Desktop/auto-ring/biblio/FondAplat.jpg", "")
...
if page_en_cours == range_page[-1]:
    if partie_en_cours != "2" :
        suivante = pdb.gimp_text_layer_new(fond, lien_direct_autre_partie, "sans", 20, 0)
    else :
...
xml_open.write("\t\t\t<button name='%d' x0='%d' y0='%d' x1='%d' y1='%d' left='%s' right='%s' up='%s' down='%s'/>\n"
                ...
                (offset_gap + precedente_width)/1.422,
...
layer2_L = pdb.gimp_file_load_layer(fond2_L, nom_export_hi[:-4] + "_temp.png")
